...
class AuthHandler
{
    ...
    public function register($parameters)
    {
        ...
        try {
            ...
            if (Module::isEnabled('addressregister')) {
                $customerId = $customer->id;
                $address = new AddressWS($this->dataHandler);
                $savedAddress = $address->saveAddress($formattedFields, null, $customerId);
            ...
            if (!Module::isEnabled('extendedregistration') &&
                ...
                !Module::isEnabled('verifycustomer')
            ) {
                if (_PS_VERSION_ > '1.7') {
            ...
            if (Module::isEnabled('verifycustomer')) {
                $verify_config = Configuration::get('verifycustomer');
                $verify_config_data = json_decode($verify_config, true);
                $disable = $verify_config_data['approve_customer'];
            }
            ...
            } elseif (Module::isEnabled('verifycustomer') && $disable == 1) {
                $this->manageVerifyCustomer($customer);
            }
    ...
    private function manageVerifyCustomer($customer)
    {
        $this->context->customer->mylogout();
        $customer->active = 0;
        $customer->update();
