...
class paiementfacturesuccesspaymentModuleFrontController extends ModuleFrontController
{
    ...
    public function initContent()
    {
        ...
        $logger->logInfo('Enter success payment | order-id=' . Tools::getValue('orderId'));
        $sql = new DbQuery();
            ...
            if ($editpayment && $editpayment->sendto_crm != 1) {
                ...
                if ($responsecheck->getStatusCode() == 200) {
                    ...
                    if (!array_key_exists('orderStatus', $successcheck)) {
                        $logger->logInfo('api smt return error paiement with message :' . $successcheck['errorMessage']);
                        ...
                        if ($successcheck['orderStatus'] == 2) {
                            $logger->logInfo('return success paiement with message :' . $message);
                            ...
                            $editpayment->message_smt = $message;
                            $editpayment->save();
                            Tools::redirect($this->context->link->getModuleLink('paiementfacture', 'errorpayment') . '?orderId=' . Tools::getValue('orderId') . '&lang=' . $langcode);
                ...
                try {
                    ...
                    $logger->logInfo('CALL CRM API to pay CRM factures : ' . $editpayment->factures);

                    if ($response->getStatusCode() == 200) {
                        $successresponse = json_decode($response->getBody(), 1);
                        if ($successresponse['success']) {
                            $logger->logInfo('Call API pay CRM factures return succsess response with message : ' . $successresponse['message']);
                            $editpayment->payment_state = 1;
                            $editpayment->sendto_crm = 1;
                            $editpayment->message = 'Payé et envoyé au CRM ';
                        } else {
                            $logger->logInfo('Call API pay CRM factures return error response with message : ' . $successresponse['message']);
                    ...
                    $logger->logInfo('Call API pay CRM factures return Exception with message : ' . $e->getMessage());
                    $editpayment->payment_state = 1;
                    $editpayment->sendto_crm = 0;
        ...
        $this->setTemplate('module:paiementfacture/views/templates/front/successpayment.tpl');
