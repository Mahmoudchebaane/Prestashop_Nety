...
* @author PrestaShop SA and Contributors <contact@prestashop.com>
  ...
  *}
  ...
  <section class="login-form">
    <form action="{$urls.pages.contact|escape:'htmlall':'UTF-8'}" method="post" {if
      $contact.allow_file_upload}enctype="multipart/form-data" {/if}>

      <header>
        <h1 class="h3">{l s='Send a message' d='Modules.Contactform.Shop'}</h1>
        <p>{l s='If you would like to add a comment about your order, please write it in the field below.'
          d='Modules.Contactform.Shop'}</p>
      </header>
      ...
      <section class="form-fields">
        ...
        <label>
          ...
          <select name="id_order">
            ...
            <option value="{$order.id_order|escape:'htmlall':'UTF-8'}">{$order.reference|escape:'htmlall':'UTF-8'}
            </option>
            {/foreach}
          </select>
        </label>
        ...
        <label>
          ...
          <textarea cols="67" rows="3"
            name="message">{if $contact.message}{$contact.message|escape:'htmlall':'UTF-8'}{/if}</textarea>
        </label>
