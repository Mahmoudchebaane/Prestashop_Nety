...
{else}
  <div class="form-group row no-gutters novform-{$field.name}{if !empty($field.errors)} has-error{/if}">
    <label
      class="{if $page.page_name =="identity" }col-md-3 {else} col-md-2{/if} form-control-label{if $field.required} required{/if}">
      {if $field.type !== 'checkbox'}
        {$field.label}
      {/if}
    </label>
    <div
      class="{if $page.page_name =="identity" }col-md-9 {else} col-md-8{/if} {if ($field.type === 'radio-buttons')} form-control-valign{/if}{if ($field.type === 'birthday')} hidden-comment{/if}">
      ...
      {elseif $field.type === 'password'}
        {block name='form_field_item_password'}
          <div class="input-group js-parent-focus">
            <input class="form-control js-child-focus js-visible-password" name="{$field.name}" type="password" value=""
              placeholder="{l s='Password' d='Shop.Forms.Labels'}" pattern=".{literal}{{/literal}5,{literal}}{/literal}"
              {if $field.required}required{/if}>
            <span class="input-group-btn">
              <button id="btn-{$field.name}" class="btn " type="button" data-action="show-password">
