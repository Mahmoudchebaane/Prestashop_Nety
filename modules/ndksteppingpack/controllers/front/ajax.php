 ...
 */
class ndksteppingpackAjaxModuleFrontController extends ModuleFrontController
{
    public function initContent()
    {
        if (Tools::getValue('token') == Tools::getToken(false)) {
            if ('getProducts' == Tools::getValue('action')) {
                return $this->ajaxGetProducts();
            }
            if ('checkCart' == Tools::getValue('action')) {
                return $this->ajaxcheckCart();
            }
            if ('categoryResult' == Tools::getValue('action')) {
                return $this->categoryResult();
            }
            if ('searchResult' == Tools::getValue('action')) {
    ...
    public function searchResult()
    {
        ...
        $context->currency->id = (int) Configuration::get(
            'PS_CURRENCY_DEFAULT'
        );
    ...
    public function categoryResult()
    {
        ...
        $blockResume = Module::getInstanceByName('ndksteppingpack');
        echo $blockResume->hookAjaxCall([
            'categorycontext' => Tools::getValue('id_category'),
            'search' => '',
        ]);
    }
    ...
    public function ajaxcheckCart()
    {
        ...
        if (
            Tools::getValue('remove_product')
            && 1 == Tools::getValue('remove_product')
        ) {
            $context->cart->updateQty(
                (int) Tools::getValue('qtty'),
                (int) Tools::getValue('productId'),
                (int) Tools::getValue('productAttributeId'),
                false,
                'down'
            );
        }
        ...
        if ($steps) {
            ...
            WHERE cp.`id_product` IN (' .
                        ...
                        ') AND id_cart = ' .
                        (int) $context->cookie->id_cart .
                        ' GROUP BY id_product ORDER BY cp.date_add'
                );
            ...
            WHERE cp.`id_product` IN (' .
                ...
                foreach ($id_products as $products) {
                    ...
                    $virtualPack[$steps[$i]['id']]['maximum'] =
                        $steps[$i]['maximum'];
                    $virtualPack[$steps[$i]['id']]['optionnal'] =
                        $steps[$i]['optionnal'];
                }

                foreach ($count_sql as $row) {
                    if (
                        $count_quantities < $steps[$i]['maximum']
                        || 0 == $steps[$i]['maximum']
                    ) {
                        if (
                            !array_key_exists(
                        ...
                        if (
                            $prod_step_qtty + $count_quantities >
                            $steps[$i]['maximum']
                        ) {
                ...
                foreach ($results[$i]['added'] as $incart) {
                    ...
                    foreach ($cart_products as $prod) {
                        ...
                        if (
                            (int) $prod['id_product'] ==
                                (int) $incart['id_product']
                            && (int) $prod['id_product_attribute'] ==
                                (int) $incart['id_product_attribute']
                        ) {
                ...
                $results[$i]['resume'] .= $blockResume->hookAjaxCall([
                    'products' => $resume_prod,
        ...
        if ($finished && Tools::getValue('create_pack')) {
            ...
            if (1 == $pack->type && $pack->id_pack_prod > 0) {
                ...
                $newPackProd->reference = Tools::str2url(
                    'Pack-' .
                        $packProd->id .
                        '-Cart-' .
                        (int) $context->cart->id
                );
                $newPackProd->supplier_reference = Tools::str2url(
                    'myndkcustomprodPack'
                );
                ...
                Db::getInstance()->delete(
                    'pack',
                    'id_product_pack = ' . (int) $newPackProd->id
                );
                ...
                foreach ($virtualPack as $vpack) {
                    ...
                    foreach ($vpack['products'] as $prod) {
                        foreach ($prod as $prodItem) {
                            ...
                            $row .=
                                ...
                                'X ' .
                                $prodItem['name'] .
                                ($prodItem['id_product_attribute'] > 0
                                    ? ' | ' . $accessorycombName
                                    : '') .
                                '; ';
                            ...
                            Db::getInstance()->execute(
                                'INSERT INTO `' .
                                    _DB_PREFIX_ .
                                    'ndksteppingpack_order` (`id_cart`, `id_product`, `id_product_attribute`, `quantity`,`id_virtual_pack`)
                                VALUES (' .
                                    (int) $context->cart->id .
                                    ', ' .
                                    (int) $prodItem['id_product'] .
                                    ', ' .
                                    (int) $prodItem['id_product_attribute'] .
                                    ', ' .
                                    (int) $maxP .
                                    ', ' .
                                    (int) $newPackProd->id .
                            ...
                            if ($prodItem['id_customization'] > 0) {
                                self::copyCustomisations(
                                    (int) $prodItem['id_customization'],
                                    (int) $newPackProd->id,
                                    $cart
                                );
                            }
                            ...
                            if ((int) $maxP > 0) {
                                ...
                                $prod_available = StockAvailable::getQuantityAvailableByProduct(
                                    $prodItem['id_product'],
                                    $prodItem['id_product_attribute']
                                );
                            }
                    ...
                    $myIdCustomization = self::addTextFieldToProduct(
                        $newPackProd->id,
                        $index,
                        1,
                        $row
                ...
                if (0 == $pack->fixed_price) {
                    ...
                    if (
                        $pack->reduction_amount > 0
                        || $pack->reduction_percent > 0
                    ) {
                    ...
                    $newPackProd->price =
                        $newPackProd->price + (float) $opt_price;
                    $newPackProd->available_for_order = 1;
                ...
                Db::getInstance()->execute(
                    ...
                    UPDATE `' .
                        _DB_PREFIX_ .
                        'product_shop` SET `available_for_order` = 1 WHERE id_product = ' .
                        (int) $newPackProd->id
                );
                Db::getInstance()->execute(
                    '
                    UPDATE `' .
                        _DB_PREFIX_ .
                        'product` SET `available_for_order` = 1 WHERE id_product = ' .
                        (int) $newPackProd->id
                );
                ...
                NdkSpack::duplicateGroupReductionCache(
                    $pack->id_pack_prod,
                    $newPackProd->id
                );
                $context->cart->updateQty(
                    ...
                    $newPackProd->id,
                    null,
                    (int) $myIdCustomization
                );

                self::setInCart((int) $myIdCustomization);
            }
        }
    ...
    public static function setInCart($id_customization)
    {
        Db::getInstance()->execute(
            '
        UPDATE `' .
    ...
    public static function copyCustomisations(
        $id_customization,
        $id_product,
        $cart
    ) {
        $result = Db::getInstance()->executeS(
            'SELECT `type`, `index`, `value` FROM `' .
                _DB_PREFIX_ .
    ...
    public static function dismount($data)
    {
        if (is_array($data) || is_object($data)) {
            $result = [];
            foreach ($data as $key => $value) {
                $result[$key] = self::dismount($value);
            }
    ...
    public static function addTextFieldToProduct(
        $id_product,
        $index,
        $type,
        $text_value
    ) {
        return self::_addCustomization(
            $id_product,
            0,
            $index,
            $type,
    ...
    public static function addPictureToProduct(
        $id_product,
        $index,
        $type,
        $file
    ) {
        return self::_addCustomization($id_product, 0, $index, $type, $file, 0);
    }

    public static function _addCustomization(
        $id_product,
