...
class VC_ajaxController extends ModuleAdminController {
	...
	public function initProcess() {
		if ( Tools::getvalue( 'hook_filter' ) ) {
			...
			$element    = $jscomposer->vc_post_param( 'element' );
			$elem_pref  = explode( '_', $element );
			...
			if ( $elem_pref[0] == 'ult' || $elem_pref[0] == 'ultimate' || in_array( $element, $pro_elemes ) ) {
				$ulti_status = Configuration::get( 'VC_ULTIMATE_STATUS' );
				if ( Module::isInstalled( 'smartultimatevcaddons' ) ) {
					$ulti_status = 1;
				}
				if ( ! $ulti_status ) {
					$this->purchaseModuleMessage( $element );
					die();
				}
			}
	...
	public function purchaseModuleMessage( $element ) {
		$html = '<h3 style="margin:30px;">'
				. 'Please <a href="' . Context::getContext()->link->getAdminLink( 'AdminJsComposerSetting' ) . '"> Activate</a> Unlimited Addons'
				. '</h3>';
