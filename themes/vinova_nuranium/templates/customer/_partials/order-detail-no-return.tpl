...
{block name='order_products_table'}
  ...
  <div class="box hidden-sm-down">
    <h3>{l s='Products' d='Shop.Theme.Checkout'}</h3>
  ...
  <h3>{l s='Products' d='Shop.Theme.Checkout'}</h3>
    {foreach from=$order.products item=product}
      <div class="order-item">
        <div class="col pl-0 pr-0"> {*row*}
          <div class="desc">{*col-sm-5 *}
            <div class="name row pl-15 pr-15">
              <p style="font-weight: 600;" class="col-xs-8">{l s='Product' d='Shop.Theme.Catalog'}:&nbsp;</p>
              <p class="col-xs-4 text-xs-right">{$product.name}</p>
            </div>
          ...
          <div class="pl-15 pr-15 ">{* col-xs-7 qty  *}
            <div>
              <div class="row  "> {* col-xs-4 *}
                <p style="font-weight: 600;" class="col-xs-8">{l s='Price' d='Shop.Theme.Catalog'}:&nbsp;</p>
                <p class="col-xs-4 text-xs-right">{$product.price}</p>
              </div>

              <div class="">{* col-xs-4 *}
                {if $product.customizations}
                  {foreach $product.customizations as $customization}
                    <p style="font-weight: 600;">{l s='Quantity' d='Shop.Theme.Catalog'}:&nbsp;</p>{$customization.quantity}
                  {/foreach}
                {else}
                  <div class='row'>
                    <p style="font-weight: 600;" class="col-xs-8">{l s='Quantity' d='Shop.Theme.Catalog'}:&nbsp;</p>
                    <p class="col-xs-4 text-xs-right">{$product.quantity}</p>
                  </div>
                {/if}
              </div>
              <div class="text-xs-right col-xs-4 d-none ">
                {$product.total}
              </div>
            </div>
          </div>
  ...
  <div class="order-totals hidden-md-up box">
    {foreach $order.subtotals as $line}
      {if $line.value}
        <div class="order-total row pr-15 pl-15">
