...
{if $displayedFacets|count}
  <div id="search_filters">
    {block name='facets_title'}
      <p class="text-uppercase h6 hidden-sm-down d-none">{l s='Filter' d='Shop.Theme.Actions'}</p>
    {/block}

    {block name='facets_clearall_button'}
      {if $activeFilters|count}
        <div id="_desktop_search_filters_clear_all" class="hidden-sm-down clear-all-wrapper">
          <button data-search-url="{$clear_all_link}" class="btn btn-tertiary js-search-filters-clear-all clearAllBtn">
      ...
      <section class="facet clearfix">
        ...
        {elseif $facet.widgetType == 'slider'}
          {block name='facet_item_slider'}
            {foreach from=$facet.filters item="filter"}
              <ul id="facet_{$_expand_id}" class="faceted-slider collapse" data-slider-min="{$facet.properties.min}"
