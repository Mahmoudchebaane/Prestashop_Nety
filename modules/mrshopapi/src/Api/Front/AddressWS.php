...
class AddressWS extends BaseWS
{
    ...
    {
        if ($this->saveAddress($params, $id, $id_customer)) {
    ...
    public function saveAddress($params, $id, $id_customer)
    {
        if (!$params || count($params) === 0) {
            ...
            if ($isEdit && (int) $address->id_customer !== (int) $id_customer) {
                ...
                if (!$cleanedParams['id_state']) {
                    $cleanedParams['id_state'] = isset($params['state']) ? (int) $params['state'] : 0;
                }
