...
class JsComposer extends Module implements WidgetInterface
{
	...
	public function __construct()
	{
		...
		$this->_url_ultimate                              = $this->_path . 'include/smartultimatevcaddons/';
		...
		$this->wpb_js_composer_automapper = array(
			'i18nLocaleVcAutomapper',
			array(
		...
		if (!$updated_ulti) {
			$this->check_latest_update();
		}
		$this->wpb_js_composer_js_view            = array(
			'i18nLocale',
			array(
				...
				'confirm_deleting_template'           => $this->l('Confirm deleting template, press Cancel to leave. This action cannot be undone.'),
				...
				'press_ok_to_delete_tab'              => $this->l('Press OK to delete tab, Cancel to leave'),
				'slide'                               => $this->l('Slide'),
				'tab'                                 => $this->l('Tab'),
				'section'                             => $this->l('Section'),
				'please_enter_new_tab_title'          => $this->l('Please enter new tab title'),
				'press_ok_delete_section'             => $this->l('Press OK to delete section, Cancel to leave'),
				...
				'inline_element_deleted'              => $this->l('deleted!'),
				'inline_element_cloned'               => $this->l('cloned.'),
		...
		$this->admin_media_path_css = _MODULE_DIR_ . 'jscomposer/include/smartultimatevcaddons/admin/css/';
		$this->admin_media_path_js  = _MODULE_DIR_ . 'jscomposer/include/smartultimatevcaddons/admin/js/';
		$this->asset_media_path     = _MODULE_DIR_ . 'jscomposer/include/smartultimatevcaddons/assets/';

		include_once $this->path('HELPERS_DIR', 'helpers_factory.php');
		include_once $this->path('HELPERS_DIR', 'helpers.php');
		include_once $this->path('CORE_DIR', 'interfaces.php');
		include_once $this->path('CORE_DIR', 'class-wpb-map.php');
		include_once $this->path('HELPERS_DIR', 'helpers_api.php');
		include_once $this->path('HELPERS_DIR', 'filters.php');
		include_once $this->path('PARAMS_DIR', 'params.php');
		include_once $this->path('SHORTCODES_DIR', 'shortcodes.php');
		if (!$this->is_admin()) {
			$this->cachedHookContent();
		}

		$this->check_ultimat_status();
	}

	private function check_ultimat_status()
	{
		$ulti_status = Configuration::get('VC_ULTIMATE_STATUS');
		if (!Module::isInstalled('smartultimatevcaddons')) {
			include_once dirname(__FILE__) . '/include/smartultimatevcaddons/modules/Ultimate_Functions.php';
		}
		if ($ulti_status) {
			$today           = date('Y-m-d');
			$check_date = Configuration::get('VC_ULTIMATE_CHECK_DATE');
			if ((strtotime($today) == strtotime($check_date))) {
				return;
			} else {
				$Smartlisence = new Smartlisence();
				$support_data = $Smartlisence->has_support();

				if ($support_data['validity'] == 0) {
					Configuration::updateValue('VC_ULTIMATE_STATUS', $support_data['validity']);
				} else {
					Configuration::updateValue('VC_ULTIMATE_STATUS', '1');
					$todate = date('Y-m-d G:i:s');
					$todate = strtotime($todate);
					$diff   = $support_data['remaining'] - $todate;
					Configuration::updateValue('VC_ULTIMATE_CHECK', $diff);
					$today           = date('Y-m-d');
			...
			if ((strtotime($today) == strtotime($check_date))) {
				...
				$Smartlisence = new Smartlisence();
				$support_data = $Smartlisence->has_support();
				if (isset($support_data['validity']) && $support_data['validity'] != '') {
					$todate = date('Y-m-d G:i:s');
					$todate = strtotime($todate);
					$diff   = $support_data['remaining'] - $todate;
					Configuration::updateValue('VC_ULTIMATE_CHECK', $diff);
	...
	public static function purchaseLinks($short_code)
	{
		switch ($short_code) {
			case 'vc_button':
				return '#';
	...
	{
		return '<a href="https://classydevs.com/slider-revolution-prestashop/?utm_source=vc&utm_medium=vc_banner&utm_campaign=vc_banner"  target="_blank"><img src="' . context::getcontext()->shop->getBaseURL() . 'modules/jscomposer/assets/images/medium-box-03.png' . '"></a>';
	}

	public static function validate()
	{
		$context = Context::getContext();
		if (isset($context->controller->admin_webpath) && !empty($context->controller->admin_webpath)) {
			return true;
		}
		die('forbidden');
	}
	...
	public function install()
	{
		$this->setVersion();
		if (
			!parent::install() || !$this->registerHook('displayBackOfficeHeader')
		...
		Configuration::updateValue('vc_load_slick_js', 'yes');
	...
	{
		$sql = array();
		include_once dirname(__FILE__) . '/include/helpers/install_sql.php';
	...
	public function moduleControllerRegistration()
	{
		...
		$vctabobj->save();
		$tab_id = $vctabobj->id;
		include_once dirname(__FILE__) . '/include/helpers/install_tab.php';
	...
	{
		$sql = array();
		include_once dirname(__FILE__) . '/include/helpers/uninstall_sql.php';
		if (is_array($sql) && !empty($sql)) {
			foreach ($sql as $s) :
	...
	public function contenthookvalue($hook = '')
	{
		...
		} elseif ($id_product = Tools::getValue('id_product')) {
	...
	public function isLoadJsComposer($loade_for = false)
	{
		...
		$modules_configuration = self::getModulesConfiguration();
	...
	public function hookDisplayAdminProductsExtra($params)
	{
		if (self::condition()) {
			...
			ob_start();
			$this->vc_include_template(
				'editors/backend_editor.tpl.php',
				array(
					'editor' => $this,
				)
			);
		...
		if (!empty($controllers)) {
			...
			foreach ($controllers as $id => $controller) {
				...
				if (Tools::getValue('controller') == 'Adminvccontentanywhere') {
					$controller['shortname'] = 'vccaw';
				} elseif (Tools::getValue('controller') == 'AdminCmsContent') {
					$controller['shortname'] = 'cms';
				}
	...
	public static function getVcInlineTag($content)
	{
		...
		$module_frontend_status = self::getModuleEditorConfiguration($controller_name, 'module_frontend_status');

		if ($module_frontend_status) {
		...
		switch ($controller_name) {
			case 'Adminvccontentanywhere':
				...
				if ($module_active_status and $module_frontend_status) {
					$vc_inline_tag = '<span id="vc_inline-anchor" style="display:none !important;"></span>';
				} else {
					$vc_inline_tag = '';
				}
	...
	public function vc()
	{
		if (!isset($this->factory['vc'])) {
			include_once $this->path('CORE_DIR', 'class-vc-base.php');
			$vc = new Vc_Base();
			include_once $this->path('EDITORS_DIR', 'popups/class-vc-templates-editor.php');
			$vc->setTemplatesEditor(new Vc_Templates_Editor());
			include_once $this->path('EDITORS_DIR', 'popups/class-vc-shortcode-edit-form.php');
			$vc->setEditForm(new Vc_Shortcode_Edit_Form());
			include_once $this->path('VENDORS_DIR', 'class-vc-vendors-manager.php');
	...
	{
		include_once $this->vc_path_dir('TEMPLATES_DIR', 'params/loop/templates.html');
	}
	...
	public function backendEditor()
	{
		if (!isset($this->factory['backend_editor'])) {
			include_once $this->path('EDITORS_DIR', 'class-vc-backend-editor.php');
	...
	public function mapper()
	{
		if (!isset($this->factory['mapper'])) {
			include_once $this->path('CORE_DIR', 'class-vc-mapper.php');
	...
	public function automapper()
	{
		if (!isset($this->factory['automapper'])) {
			include_once $this->path('SETTINGS_DIR', 'class-vc-automapper.php');
	...
	public function updater()
	{
		if (!isset($this->factory['updater'])) {
			include_once $this->path('UPDATERS_DIR', 'class-vc-updater.php');
			$updater = new Vc_Updater();
			include_once $this->vc_path_dir('UPDATERS_DIR', 'class-vc-updating-manager.php');
	...
	public function settings()
	{
		if (!isset($this->factory['settings'])) {
			include_once $this->path('SETTINGS_DIR', 'class-vc-settings.php');
	...
	{
		extract($args);
		include $this->vc_path_dir('TEMPLATES_DIR', $file);
	}
	...
	public function renderEditorFooter()
	{
		...
		$this->init();
		$this->vc_include_template(
			'editors/partials/backend_editor_footer.tpl.php',
			array(
				'editor' => $this,
			)
		);
	...
	public function frontendEditor()
	{
		if (!isset($this->factory['frontend_editor'])) {
			include_once $this->path('EDITORS_DIR', 'class-vc-frontend-editor.php');
	...
	public static function delete_uploaded_file()
	{
		...
		include_once dirname(__FILE__) . '/views/lang/en.php';
	...
	public function check_latest_update()
	{
		$lincensing = new Smartlisence();
		$lincensing->updating_js();
	}
	public static function get_uploaded_files_result($per_page = 20, $start = 0, $subdir = '')
	{
		...
		if (!empty($db_results)) {
			foreach ($db_results as $dres) {
				...
				if (
					isset($dres->file_name) && !empty($dres->file_name) && file_exists(vc_manager()->composer_settings['UPLOADS_DIR'] . $dres->subdir . $dres->file_name)
				) {
...
?>
	...
	public function wpb_single_image_src()
	{
		if (Tools::getValue('content') && is_numeric(Tools::getValue('content'))) {
			...
			echo self::ModifyImageUrl($image_src);
			die();
		}
	}
	...
	public static function import_media_img($file_url, $folder, $filename, $imgSubDir = '')
	{
		...
		$q = '';
		if ($imgSubDir == '') {
			$q = "SELECT COUNT(*) AS found FROM {$tablename} WHERE file_name LIKE '{$tempname}%' AND subdir IS NULL";
		} else {
			$q = "SELECT COUNT(*) AS found FROM {$tablename} WHERE file_name LIKE '{$tempname}%' AND subdir='{$imgSubDir}'";
		}
		$found = $db->getValue($q);
		if ($found && $found > 0) {
				...
				case 'smartblog':
					if (class_exists('SmartBlogPost')) { // smartblog link generator...
						...
						if (!empty($blog->id_smart_blog_post)) {
							$options = array(
								'id_post' => $blog->id_smart_blog_post,
								'slug'    => $blog->link_rewrite[intval($id_lang)],
							);
	...
	{
		$smarty = new smarty();
		self::fronted_smarty_asign($smarty);
		return $smarty;
	}
	...
	public static function fronted_smarty_asign($smarty)
	{
		...
		if ((int) Context::getcontext()->cookie->id_cart) {
			...
			/* Delete product of cart, if user can't make an order from his country */ elseif (
				intval(Configuration::get('PS_GEOLOCATION_ENABLED'))
			...
			if (
				isset($cart) && (!isset($cart->id_address_delivery) || $cart->id_address_delivery == 0
	...
	public function vcallmod()
	{
		...
		if ($this->is_admin()) {
			if (Tools::getValue('action') == 'wpb_show_edit_form') {
				...
				if (isset($params['execute_module'])) {
					$GetAllmodules_list[] = array(
						'id'   => $params['execute_module'],
						'name' => $params['execute_module'],
					);
				}
			} else {
				$GetAllmodules_list = $vccaw->GetAllFilterModules();
		...
		if (!empty($GetAllmodules_list)) {
			foreach ($GetAllmodules_list as &$value) {
				...
				self::add_shortcode('vc_' . $value['id'], array($this, 'vcallmodcode'));
				if ($this->is_admin()) {
					if (is_object($vccaw)) {
	...
	public function hookDisplayHeader($params)
	{
		...
		if (Configuration::get('vc_load_slick_js') != 'no') {
	...
	public function hookDisplayBackOfficeHeader()
	{
		...
		if (Tools::version_compare(_PS_VERSION_, '1.7.6.0', '>=')) {
			...
			if (Tools::getValue('controller') == 'AdminCmsContent') {
				...
				$link_array = explode('design/cms-pages/category', $actual_link);

				if (count($link_array) > 1) {
					Media::addJsDef(array(
						'pageType' => 'cmsCategory',
						'ad' => __PS_BASE_URI__ . $this->context->controller->admin_webpath,
			...
			if (Tools::getValue('id_cms_category')) {
				Media::addJsDef(array(
					'pageType' => 'cmsCategory'
				));
			} else {
		...
		if (self::condition()) {
			...
			$content_local  = 'var ' . $this->wpb_js_composer_js_view[0] . '=' . Tools::jsonEncode($this->wpb_js_composer_js_view[1]) . ";\n";
			$content_local = stripslashes($content_local);
			$content_mapper = 'var ' . $this->wpb_js_composer_automapper[0] . '=' . Tools::jsonEncode($this->wpb_js_composer_automapper[1]) . ";\n";
			$content_mapper = stripslashes($content_mapper);
			$content_mapper .= "\n";


			$content_etc = 'var vcActiveTextFieldName ="' . $vcActiveTextFieldName . "\";\n";

			if (Tools::getValue('controller') == 'AdminProducts') {
				$content_etc .= 'var vcActiveTextFieldName ="form_step1_description' . "\";\n";
			} else {
				$content_etc .= 'var vcActiveTextFieldName ="' . $vcActiveTextFieldName . "\";\n";
			}

			$content_etc .= 'var vc_ajaxurl="' . $this->ajaxController . "\";\n";
			$content_etc .= 'var wpColorPickerL10n=' . Tools::jsonEncode(
				array(
					'clear'         => $this->l('Clear'),
			...
			$content_etc .= 'var vc_mediaAjaxUrl = \'' . self::controller_upload_url() . "';\n";
			$content_etc .= "var vc_urlImage = '{$modulepath}';\n";
			$content_etc .= 'var id_lang = ' . (Tools::getValue('id_lang') ? Tools::getValue('id_lang') : $this->context->language->id) . ";\n";
			$content_etc .= self::$vcBackofficePageIndenfiers . "\n";
			if (Tools::getValue('controller') == 'VC_frontend') {
			...
			if (Tools::getValue('controller') != 'VC_frontend') {
				$content_etc .= '$(function(){$("body").addClass("vc_backoffice");});' . "\n";
			}
			$content_vars = 'var vc_mapper = ' . Tools::jsonEncode(WPBMap::getShortCodes()) . ";\n";

			$content_var = "";
			$content_var .= "var vc_mode = '{$this->mode}';\n";
			...
			$content_var .= $frontend_stat . $backend_stat;

			if (
				Tools::version_compare(_PS_VERSION_, '1.7.6.0', '>=')
				...
				if (Tools::getValue('controller') != 'AdminProducts') {
					$this->context->controller->addCSS(__PS_BASE_URI__ . $admin_webpath . '/themes/default/public/theme.css');
				}
				if (Tools::getValue('controller') == 'VC_frontend') {
				...
				if (Tools::version_compare(_PS_VERSION_, '1.7.8.0', '<')) {
					$this->context->controller->addJS(_PS_JS_DIR_ . 'jquery/jquery-3.4.1.min.js');
				}
			}
			...
			$paginationPage = Tools::getValue('controller');
			if ($paginationPage != 'AdminProducts') {
			...
			// $content_vars = stripslashes($content_vars);
			$content_etc = stripslashes($content_etc);
			echo "<script type='text/javascript'>{$content_local}{$content_mapper}{$content_etc}{$content_var}{$content_vars}</script>";
			...
			<script type="text/javascript">
				...
				function sds_id_lang(lang_code = null, on_page_load = 0) {
					if (on_page_load) {
						...
						$id_lang = (int) Context::getContext()->language->id;
						?>
						var returnlang = <?php echo $id_lang; ?>;
						...
						if (lang_code == null) {
							if (pageType == 'cms') {
								...
								var lang_code = $('#cms_page_category_meta_title').html();
								lang_code = $.trim(lang_code);
				...
				$(function() {
					$(document.body).on('focusout', '.translatable-field ul.dropdown-menu li a', function() {
						...
						lang_code = lang_code.replace("javascript:hideOtherLanguage(", "");
						lang_code = lang_code.replace(");", "");
						$("#vclangID").val(lang_code);
						changeFrontEditorLang();
					});

					$(document.body).on('click', '.js-locale-item', function() {
						var lang_code = $(this).attr("data-locale");
						var langval = window.lang_for_1_7_6(lang_code);
						$("#vclangID").val(langval);
						changeFrontEditorLang(lang_code);
					});
					$(document.body).on('click', '.translationsLocales li a', function() {
						var lang_code = $(this).attr("data-locale");
						var langval = window.lang_for_1_7_6(lang_code);
						$("#vclangID").val(langval);
						changeFrontEditorLang(lang_code);
...
<?php
	...
	public function hookDisplayBackOfficeFooter()
	{
		...
		if (Tools::version_compare(_PS_VERSION_, '1.7.5.0', '>=')) {
			if (Tools::getValue('controller') == 'AdminProducts') {
				...
				$paginationPage = Tools::getValue('controller_name');

				if (isset($url_arr[1]) && !isset($_GET['controller_name'])) {
					if ($paginationPage != 'PrestaShopBundle\Controller\Admin\ProductController') {
		...
		if (self::condition()) {
			...
			ob_start();
			if (isset(self::$sds_action_hooks['ps_admin_footer'])) {
				call_user_func(self::$sds_action_hooks['ps_admin_footer']);
			}
	...
	public static function AddVcExternalControllers($attribs)
	{
		...
		*/
		if (!empty($attribs)) {
			...
			if ($key) {
				$controllers[$key]['module_status'] = '1';
			}
			if ($key) {
				$controllers[$key]['module_frontend_status'] = '0';
			}
			if ($key) {
				$controllers[$key]['module_backend_status'] = '0';
			}
			if ($key) {
				$controllers[$key]['type'] = 'custom';
			}
	...
	public static function ultimate_hex2rgb($hex, $opacity = 1)
	{
		$hex = str_replace('#', '', $hex);
		if (strlen($hex) == 3) {
	...
	public function DoAction($ac_type = 'addcode')
	{
		$includes    = array();
		$ulti_status = Configuration::get('VC_ULTIMATE_STATUS');
		...
		if ($ulti_status) {
			include_once dirname(__FILE__) . '/include/smartultimatevcaddons/inc/include.php';
		}

		if (isset($includes) && !empty($includes)) {
			foreach ($includes as $key => $include) {
				if (isset($include['file']) && isset($include['class'])) :
					$fileuri = dirname(__FILE__) . '/include/smartultimatevcaddons/modules/' . $include['file'];
	...
	// public static function getTPLPath($template = '', $module_name = 'smartultimatevcaddons') {
	// if (Tools::file_exists_cache(_PS_THEME_DIR_ . 'modules/' . $module_name . '/' . $template))
	// return _PS_THEME_DIR_ . 'modules/' . $module_name . '/' . $template;
	// elseif (Tools::file_exists_cache(_PS_THEME_DIR_ . 'modules/' . $module_name . '/views/templates/front/' . $template))
	// return _PS_THEME_DIR_ . 'modules/' . $module_name . '/views/templates/front/' . $template;
	...
	// return false;
	// }
	public static function plugins_url($params = '', $filess = '')
	{
		$url = __PS_BASE_URI__ . 'modules/smartultimatevcaddons/' . $params;
		return $url;
		// smartultimatevcaddons::plugins_url();
	}

	public function include_plugins_module_file($file_name)
	{
		include_once dirname(__FILE__) . '/include/smartultimatevcaddons/' . $file_name;
	}

	public static function get_module_dir($extrapath = '')
	{
		return dirname(__FILE__) . '/' . $extrapath;
	}
}
