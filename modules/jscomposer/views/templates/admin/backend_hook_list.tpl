...
$(document.body).on('click', '.activate_ulti' ,function(){


	var url_action_name = 'change_status';
	var url_post_type = 'ajax';
	var url_module_name = $(this).attr("data-js_module_name");
	var ulti_status = $(this).attr("data-js_ulti_status");
	...
	$.ajax({
	  type: 'POST',
	  url: '{$url_admin_ajax}',
	  data: 'method=changeUltimateStatus&module_name='+url_module_name+'&ulti_status='+ulti_status,
	  success: function(json) {
		json = JSON.parse(json);
	  	if(json.status){
			$('.msg_ulti').html(json.msg)
			$('.msg_ulti').addClass('active_ulti')
			$(this).data( "js_ulti_status", '1' );
			$('.not-activated').removeClass('show_active')
			$('.not-activated').addClass('hide_active')
			$('.ulti-activated').removeClass('hide_active')
			$('.ulti-activated').addClass('show_active')
			$('#activateUltimate').val('Deactivate Ultimate Addons')
		} else {
			$('.msg_ulti').html(json.msg)
			$('.msg_ulti').addClass('warning_ulti')
			$('.not-activated').removeClass('hide_active')
			$('.not-activated').addClass('show_active')
			$('.ulti-activated').removeClass('show_active')
			$('.ulti-activated').addClass('hide_active')
			$('#activateUltimate').val('Activate Ultimate Addons')
			$(this).data( "js_ulti_status", '0' );
		}
	  }
	});
});
