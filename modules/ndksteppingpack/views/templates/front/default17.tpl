...
<script>
 ...
 var ndksp_auto_timeline = '{Configuration::get('NDKSTEPPINGPACK_AUTO_TIMELINE', 0)}';
</script>
...
{if $pack->fixed_price == 0 && ($pack->reduction_amount > 0 || $pack->reduction_percent > 0)}
<p class="discount">
	{l s='You save' mod='ndksteppingpack'}
	{if $pack->reduction_amount > 0}
...
<ul class="stepnav">
	{foreach from=$steps item=step name='steps'}
		<li id="stepNav_{$step.id|escape:'html':'UTF-8'}" class="stepnavButton  {if $smarty.foreach.steps.first} activeStep firstStep{/if} {if $smarty.foreach.steps.index > 0} disabled{/if} {if $step.optionnal == 1} optionnalStep{/if}" {if $smarty.foreach.steps.index > 0} disabled="disabled"{/if} data-target="step_{$step.id|escape:'html':'UTF-8'}" data-id-step="{$step.id|escape:'html':'UTF-8'}">
			<span class="number">{$smarty.foreach.steps.iteration}</span>
			<p>{$step.name|escape:'html':'UTF-8'}</p>
		</li>
	{/foreach}
	<li id="stepNav_order" class="">
		<a id="stepNav_999" disabled="disabled" class="button button-medium disabled" href="{$cart_url}" title="{l s='Proceed to checkout' mod='ndksteppingpack'}" rel="nofollow">
			<span class="material-icons fa fa-shopping-bag fa-fw icon"></span>
			<p>{l s='Proceed to checkout' mod='ndksteppingpack'}</p>
		</a>
	</li>
...
<div id="timeline">
	...
	<div id="resume_pack" class="clearfix row">
		...
		{foreach from=$steps item=step name='steps'}
			<div class="resume-step " id="resume-step-{$step.id|escape:'html':'UTF-8'}">
				<span class="number">{$smarty.foreach.steps.iteration}</span>
...
<div class="filters_ps_facetedsearch">{*$ps_facetedsearch*}</div>
{foreach from=$steps item=step name='steps'}
	...
	<div class="packIsFull alert alert-success">
		<h4>{l s='You reach the maximum products number in this step' mod='ndksteppingpack'}</h4>
		{if $smarty.foreach.steps.last}
		<a class="continue btn btn-default button exclusive-medium btn-primary" href="#"  onclick="simulateLinkFromPopup(999);"><span>{l s='Proceed to checkout' mod='ndksteppingpack'}</span></a>
		{else}
		<a class="continue btn btn-default button exclusive-medium btn-primary" href="#" onclick="simulateLinkFromPopup({$steps[$nextindex].id});"><span>{l s='Go to the next step' mod='ndksteppingpack'}</span></a>
		{/if}
	</div>
