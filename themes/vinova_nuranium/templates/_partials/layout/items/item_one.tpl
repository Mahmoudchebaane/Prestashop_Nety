...
<div class="item {$class_item}">
	<div class="product-miniature js-product-miniature item-one lupanh{if $smarty.foreach.products.first} first_item{elseif $smarty.foreach.products.last} last_item{/if}" data-id-product="{$product.id_product}" data-id-product-attribute="{$product.id_product_attribute}" 
					...
					{if isset($novconfig.novthemeconfig_cat_product_addtocart) && $novconfig.novthemeconfig_cat_product_addtocart == 1 }
						{if $product.quantity > 0}
							...
							{else}
								<form action="{$link->getPageLink('cart')|escape:'html'}" method="post">
									...
									<button class="add-to-cart show_popup btn" href="#" data-button-action="add-to-cart" type="submit">
						...
						{else}
							<form action="{$link->getPageLink('cart')|escape:'html'}" method="post">
								...
								<input type="hidden" name="id_product" value="{$product.id}">
								<input type="hidden" name="qty" value="{$product.minimal_quantity}">
								<button class="add-to-cart show_popup btn" href="#" data-button-action="add-to-cart" type="submit" {if !$product.add_to_cart_url} disabled {/if}>
									<span class="loading"><i class="fa fa-spinner fa-spin"></i></span>
