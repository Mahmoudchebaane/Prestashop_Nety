...
class PaymentHandler
{
    ...
    public function findValidPaymentMethods($paymentMethodsToValidate = null)
    {
        $modulesArray = [];

        $allowedPaymentMethods = MrShopPaymentModule::getAllowedPaymentModules();

        foreach ($allowedPaymentMethods as $allowedPaymentMethod) {
            if (empty($paymentMethodsToValidate) || in_array($allowedPaymentMethod['name'], $paymentMethodsToValidate)) {
                try {
                    if (version_compare(_PS_VERSION_, '1.7.0', '<')) {
                        $paymentModuleHtml = Hook::exec('displayPayment', [], $allowedPaymentMethod['id_module'], true);
                        if (!empty($paymentModuleHtml)) {
                            $modulesArray[] = $allowedPaymentMethod['name'];
                        }
                    } else {
                        $paymentOptions = Hook::exec('paymentOptions', [], $allowedPaymentMethod['id_module'], true);
    ...
    public function getValidPaymentModulesName($customerId, $cart = null)
    {
        ...
        return $this->findValidPaymentMethods();
    }
    ...
    {
        $paymentMethods = $this->findValidPaymentMethods([$paymentMethodName]);
