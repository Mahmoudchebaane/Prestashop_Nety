...
class demandeabonnementnouveauModuleFrontController extends ModuleFrontController
{
    ...
    public function initContent()
    {
        ...
        if (Tools::isSubmit('submit')) {             
            ...
            if (!count($this->errors)) { 
                    ...
                    try {
                        ...
                        $response = $client->post($linkcrm . '/api/adddemandeabonnement', $options);
        ...
        $this->setTemplate('module:demandeabonnement/views/templates/front/formulaireDA.tpl');        
    ...
    public function displayAjaxCheckCode()
    {
        ...
        $check = false;
        // $message = $this->trans("une erreur se produite");
        // $message = $this->l('une erreur se produite');
            $message= 1;
            // $this->trans(
            //     'une erreur se produite',
            //     [get_class($this) . '->' . 'une erreur se produite'],
            //     'Modules.demandeabonnement.Shop'
            // );   
            ...
            if ($tel == $this->context->cookie->__get('tel')) {
                if ($codesms == $this->context->cookie->__get('code')) {
                    ...
                    if (($now - $expire) < 0) {
                        ...
                        //     [get_class($this) . '->' . 'Code de vérification valide'],
                        //     'Modules.demandeabonnement.Shop'
                        // ); 
                        // $this->l("Code de vérification valide");
                    } else {
                        // $message = $this->trans("Code de vérification expiré", [], Modules.demandeabonnement.Shop);
                        $message= 3;
                       
                    }
                } else {
                    //  $message = $this->l("Code de vérification invalide");
                    $message= 4;
                }
            }
            $this->ajaxRender(
