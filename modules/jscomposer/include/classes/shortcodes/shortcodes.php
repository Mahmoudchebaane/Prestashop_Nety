...
if ( ! class_exists( 'WPBakeryShortCode_UniversalAdmin' ) ) {
	abstract class WPBakeryShortCode_UniversalAdmin extends WPBakeryShortCode {
		...
		protected function singleParamEditForm( $param, $param_value ) {
			...
			} elseif ( $param['type'] == 'taxonomies' || $param['type'] == 'taxomonies' ) {
			...
			elseif ( $param['type'] == 'textarea_safe' ) {
				// $param_value = $param_value;
		...
		protected function getTinyHtmlTextArea( $param = array(), $param_value ) {
			...
			if ( function_exists( 'wp_editor' ) ) {
				...
				wp_editor(
					$default_content,
					'wpb_tinymce_' . $param['param_name'],
					array(
						'editor_class'  => 'wpb_vc_param_value wpb-textarea visual_composer_tinymce ' . $param['param_name'] . ' ' . $param['type'],
						'media_buttons' => true,
						'wpautop'       => true,
					)
				);
	...
	abstract class WPBakeryShortCodesContainer extends WPBakeryShortCode {
		...
		public function getColumnControls( $controls, $extended_css = '' ) {
			...
			if ( $extended_css == 'bottom-controls' ) {
				$control_title = sprintf( $vc_manager->l( 'Append to this %s' ), strtolower( $this->settings( 'name' ) ) );
			} else {
				$control_title = sprintf( $vc_manager->l( 'Prepend to this %s' ), strtolower( $this->settings( 'name' ) ) );
			}
