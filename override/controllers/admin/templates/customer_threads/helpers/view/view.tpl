...
{block name="override_tpl"}
    ...
    <div class="panel">
        ...
        <div class="well">
            <form
                action="{$link->getAdminLink('AdminCustomerThreads', true, [], ['id_customer_thread' => $id_customer_thread|intval, 'viewcustomer_thread' => 1])|escape:'html':'UTF-8'}"
        ...
        <div class="row">
            <div class="message-item-initial media">
                <a href="{if isset($customer->id)}{$link->getAdminLink('AdminCustomers', true, [], ['id_customer' => $customer->id|intval, 'viewcustomer' => 1])|escape:'html':'UTF-8'}{else}#{/if}"
                    class="avatar-lg pull-left"><i class="icon-user icon-3x"></i></a>
                <div class="media-body">
                    <div class="row">
                        <div class="col-sm-6">
                            {if isset($customer->firstname)}
                                <h2>
                                    <a
                                        href="{$link->getAdminLink('AdminCustomers', true, [], ['id_customer' => $customer->id|intval, 'viewcustomer' => 1])|escape:'html':'UTF-8'}">
                                        {$customer->firstname|escape:'html':'UTF-8'} {$customer->lastname|escape:'html':'UTF-8'}
                                        <small>({$customer->email|escape:'html':'UTF-8'})</small>
                                    </a>
                                </h2>
                            {else}
                            ...
                            {if isset($contact) && trim($contact) != ''}
                                <span>{l s="To:" d='Admin.Orderscustomers.Feature'} </span><span
                                    class="badge">{$contact|escape:'html':'UTF-8'}</span>
                            {/if}
                        </div>
                        {if isset($customer->firstname)}
                            <div class="col-sm-6">
                                ...
                                <p class="text-muted">
                                    ...
                                    d='Admin.Orderscustomers.Feature'}</p>
                            </div>
                        {/if}
                    </div>
                    <div class="row">
                    <div class="col-sm-12">
                        ...
                        {* {if !$first_message.id_employee}
                           {$first_message|json_encode}
                            {include file="./message.tpl" message=$first_message initial=true}
                        {/if} *}
                        {$last_message=end($messages)}

                        {if $last_message}
                            {if !$last_message.id_employee}
                                {include file="./message.tpl" message=$last_message initial=true}
                            {/if}
                        {else}
                            {if !$first_message.id_employee}
        ...
        <div class="row">
            ...
            {foreach $messages|@array_reverse as $message}
                {include file="./message.tpl" message=$message initial=false}
            {/foreach}
        </div>
    </div>
    <div class="panel">
        <h3 id="reply-form-title">{l s="Your answer to" d='Admin.Orderscustomers.Feature'}
            {if isset($customer->firstname)}{$customer->firstname|escape:'html':'UTF-8'}
            {$customer->lastname|escape:'html':'UTF-8'} {else} {$thread->email}
            {/if}</h3>
        <form
            action="{$link->getAdminLink('AdminCustomerThreads', true, [], ['id_customer_thread' => $thread->id|intval, 'viewcustomer_thread' => 1])|escape:'html':'UTF-8'}"
            method="post" enctype="multipart/form-data" class="form-horizontal">
            <div class="row">
                <div class="media">
                    <div class="pull-left">
                        <span class="avatar-md">{if isset($current_employee->firstname)}<img
                                src="{$current_employee->getImage()}" alt="">{/if}</span>
                    </div>
                    <div class="media-body">
                        <textarea id="reply_message" cols="30" rows="7"
                            name="reply_message">{$PS_CUSTOMER_SERVICE_SIGNATURE|escape:'html':'UTF-8'}</textarea>
                    </div>
                </div>
            </div>
			...
			{l s="Choose a template"}
                ...
                <button class="btn btn-default pull-right" name="submitReply"><i class="process-icon-mail-reply"></i>
                    {l s="Send"}</button>
