...
class ProductBaseWS extends BaseWS
{
    ...
    public function getAttributeTextureLink($idAttribute)
    {
        if (@file_exists(_PS_COL_IMG_DIR_.$idAttribute.'.jpg')) {
            $uriPath = _THEME_COL_DIR_.$idAttribute.'.jpg';
    ...
    private function setupAttribute($attribute)
    {
        $item = [
            ...
            'texture' => $this->getAttributeTextureLink($attribute['id_attribute']),
            'color' => null
        ];

        if (!$item['texture'] && $attribute['color'] !== '' && $attribute['color'] !== null) {
        ...
        foreach ($attributes as $attribute) {
            ...
            foreach ($groups as &$group) {
                if ($group['id_group'] == $attribute['id_attribute_group']) {
                    $item = $this->setupAttribute($attribute);
